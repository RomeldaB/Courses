...
def harris_corners(img, window_size=3, k=0.04):
    ...
    # YOUR CODE HERE
    # Derivatives product
    Ix_squared = convolve(dx * dx, window)
    Iy_squared = convolve(dy * dy, window)
    Ix_y = convolve(dx * dy, window)
    a = b = 0
    while a < H:
        while b < W:
            M = np.array([[Ix_squared[a][b], Ix_y[a][b]], [Ix_y[a][b], Iy_squared[a][b]]])
            # Eigen values/vectors
            lambda_1, lambda_2 = np.linalg.eig(M)
            x, y = lambda_1[0], lambda_1[1]
            response[a][b] = x * y - k * (x + y) * (x + y)
            b += 1
        b = 0
        a += 1
    # END YOUR CODE
...
def simple_descriptor(patch):
    ...
    a_mean = np.average(feature)
    standard_deviation = np.std(feature)
    if(standard_deviation == 0):
        standard_deviation = 1
    # Change value of feature
    feature = (feature - a_mean) / standard_deviation
    # END YOUR CODE
    return feature
...
def match_descriptors(desc1, desc2, threshold=0.5):
    ...
    k = 0
    while k < N:
        threshold_check = np.min(dists,1)[k]/dists[k] > threshold;
        if(np.sum(threshold_check)<=1):
            matches.append(np.array([k, np.argwhere(threshold_check==True)[0][0]]))
        k += 1
...
def ransac(keypoints1, keypoints2, matches, n_iters=200, threshold=20):
    ...
    # YOUR CODE HERE
    # Maximum count value and inliers
    max_index = 0;
    max_inliers = list();
    ...
    while k < n_iters:
        ...
        rand = np.random.randint(0,n_matches,n_samples);
        
        rand1, rand2 = matched1[rand], matched2[rand]
        
        # Affine trans_matrix
        H = np.linalg.lstsq(rand2, rand1)[0]
        l = 0
        while l < np.shape(H)[0]:
            if(l != np.shape(H)[0]-1):
                H[l,-1]=0;
            else:
                H[l,-1]=1;
        ...
        m = 0
        while m < n_matches:
            abs_norm = np.abs(np.linalg.norm(matched2[m]*H,ord=None) - np.linalg.norm(matched1[m],ord=None))
            if(abs_norm < threshold):
                inliers.append(m)
                index += 1
            m += 1
        # Select the maximal set of inliers   
        if(max_index < index):
            max_index = index
            max_inliers = inliers
        k += 1
    
    # Recompute least-squares estimate on all of the inliers
    H = np.linalg.lstsq(matched2[max_inliers], matched1[max_inliers])[0];
    k = 0
    while k < np.shape(H)[0]:
        if(k != np.shape(H)[0]-1):
            H[k,-1] = 0
        else:
            H[k,-1] = 1
...
def hog_descriptor(patch, pixels_per_cell=(8, 8)):
    ...
    while k < rows:
        while l < cols:
            while m < G_cells.shape[2]:
                while n < G_cells.shape[3]:
                    angle = theta_cells[k, l, m, n] + 180
                    angle = int(angle *n_bins)//360
                    if(angle == 9):
                        angle = 0;
                    cells[k, l, angle] += G_cells[k, l, m, n];
                    n += 1
                n = 0
                m += 1
            m = 0
